---
title: Kaboom.js object makers made easy
description: The new way for creating objects in Kaboom.js
template: splash
hero:
  tagline: The new way for creating objects in Kaboom.js, object makers as a prefab system for your game
  image:
    file: ../../assets/kiboombanner.png
    alt: Kiboom logo
    
  actions:
    - text: Get started
      link: /start
      icon: right-arrow
      variant: primary
    - text: Reference
      link: https://kiboom-ref.pages.dev/
      icon: external
      variant: secondary
---

import { Code } from "@astrojs/starlight/components";
import { Card, CardGrid } from '@astrojs/starlight/components';


<CardGrid>
  <Card title="Kaboom-like" icon="star">
    Using the functional approach of Kaboom.js, kiboom plugin feels like a native part 
    of the library.
  </Card>
  <Card title="Extendable API" icon="puzzle">
    Create, merge and extend your object makers for create optionable objects
  </Card>
  <Card title="Easy to use" icon="rocket">
    Create your own maker functions with a few lines of code.
  </Card>
  <Card title="TypeScript Support" icon="seti:typescript" >
    Full TypeScript support out of the box.
  </Card>
</CardGrid>

## Example

export const exampleCode = `
// Create a enemy maker

// We create some options with defaults
const makeEnemyOpt = makeOptions(() => ({
    speed: 100,
    hp: 100,
}));

// We create the new maker
const makeEnemy = extendMaker(makeObject, makeEnemyOpt, (opt) => [
    // here our components list
    hp(opt.hp),
    move(LEFT, opt.speed),
]);

// Now we can use our new maker
k.add(makeEnemy({
    hp: 200,             // We can override our maker options
    speed: 200,          // We can override our maker options
    pos: vec2(100, 100), // We can modify the base options of makeObject
    anchor: "topleft",   // We can modify the base options of makeObject
}));
`;
export const fileName = 'example.js';
export const highlights = ['file', 'CMS'];

<Code code={exampleCode} lang="js" title={fileName} mark={highlights} />
